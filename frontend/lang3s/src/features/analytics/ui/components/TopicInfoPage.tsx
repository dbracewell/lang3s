...
import { topicsGetTopicOptions } from "@/clients/core/@tanstack/react-query.gen";
import { coreClient } from "@/lib/api";
import { Spinner } from "@/components/Spinner";
import { cn } from "@/lib/utils/cn";
import { Hint } from "@/components/hint";

export const TopicInfoPage = ({ id }: { id: number }) => {
  const { data, error } = useQuery({
    ...topicsGetTopicOptions({
      client: coreClient,
      path: {
        topic_id: id,
      },
        ...
        <CardHeader>
          ...
          <CardDescription>
            <h2 className="text-muted-foreground text-md pb-5">
              {new Intl.NumberFormat().format(data.sentence_count ?? 0)}{" "}
              sentences across{" "}
              {new Intl.NumberFormat().format(data.document_count ?? 0)}{" "}
              documents.
            </h2>
          </CardDescription>
          <CardAction>
