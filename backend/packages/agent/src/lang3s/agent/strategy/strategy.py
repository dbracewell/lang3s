...
class StrategyResult(Generic[STRATEGY_RESPONSE_TYPE]):
    ...
    def update(self, agent_event: AgentEvent[STRATEGY_RESPONSE_TYPE]) -> None:
...
class Strategy(ABC, Generic[STRATEGY_RESPONSE_TYPE]):
    ...
    async def _chat_to_completion(
        ...
        result: AgentEvent[STRATEGY_RESPONSE_TYPE] | None = None
    ...
    async def _async_chat(
        ...
        **kwargs: Unpack[ChatCompletionParams],
        ...
        if "max_completion_tokens" not in kwargs:
            kwargs["max_completion_tokens"] = session.context_window

        async for event in session.client.chat(
            ...
            **kwargs,
            ...
            if event.type == LLMEventType.TOOL_CALL_COMPLETE:
                ...
                agent_event = AgentEvent.text_delta_event(event.content or "")
            elif event.type == LLMEventType.COMPLETE:
                agent_event = AgentEvent.text_complete_event(
                    event.content or "", event.parsed
                )
                if event.content:
                ...
                agent_event = AgentEvent.error_event(event.exception)  # type: ignore
