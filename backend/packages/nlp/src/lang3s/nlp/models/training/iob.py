...
from lang3s.nlp.models.task_transformer.heads import TokenClassificationHead
from lang3s.nlp.models.task_transformer.task import (
    TokenClassificationParams,
    repair_bio_seq,
)
from lang3s.nlp.models.task_transformer.typedefs import TaskType
...
class IObTrainer(Trainer):
    ...
    def _train_impl(self):
        ...
        optimizer = torch.optim.AdamW(
            self.clf.parameters(), lr=self.lr, weight_decay=0.01
        )
        ...
        for epoch in tqdm(range(self.num_epochs), disable=not self.is_trial):
            ...
            if score > best_score + 1e-4:
                logger.info(
                    ...
                    f"Token-F1 {best_token_f1:.4f} => {token_f1:.4f}\nSaving model..."
                )
    ...
    def eval_one_epoch(self) -> Dict[str, Any]:
        ...
        t_prec, t_rec, t_f1, _ = precision_recall_fscore_support(
            flatten(all_gold), flatten(all_pred), average="macro", zero_division=0
        )
    ...
    def print_metrics(self, metrics: Dict[str, Any], epoch=-1, file=sys.stdout):
        ...
        for tokens, gold_seq, pred_seq in zip(sample_text, sample_gold, sample_pred):
            ...
            print(
                "Gold:       ",
                " ".join(f"{lbl:<{longest_label}}" for lbl in gold_seq),
                file=file,
            )
            print(
                "Predicted:  ",
                " ".join(f"{lbl:<{longest_label}}" for lbl in pred_seq),
                file=file,
            )
            print("-" * 80, file=file)
    ...
    def train_one_epoch(
        ...
        for batch in tqdm(
            range(len(self.train_dataloader)), disable=self.is_trial, desc="  Training"
        ):
    ...
    def prepare_batch(
        ...
        embeddings = (
            torch.from_numpy(padded_embeddings).type(torch.float32).to(self.device)
        )
