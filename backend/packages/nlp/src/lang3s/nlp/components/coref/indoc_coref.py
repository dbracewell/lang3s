...
from lang3s.nlp.models.coref_ranker import (
    REVERSE_POS_MAP,
    FastCorefRanker,
    create_coref_mention,
)

from .helper import should_perform_coref
...
def _apply_nominal_sieve(mention, candidates, all_scores, threshold=-1.5):
    ...
    if (
        ...
        # Both are person and neither are pronouns or titles,
        # so they have to have some overlap
    ...
    for i in range(len(candidates) - 1, -1, -1):
        ...
        # ---------------------------------------------------------
        # Rule A: Sub-string overlap for Proper Nouns
        # (e.g., "Eliot Spitzer" -> "Mr Spitzer")
        # If one string is entirely contained within the other,
        # and both are PROPN/Persons.
    ...
    # If no candidates pass the sieve, accept the network's decision
    # to leave it unlinked
    return predicted_idx
...
class InDocumentCoref:
    ...
    def load_coref_model(cls, device="cpu"):
        ...
        """
        filepath = config.MODELS_DIR / "coref-old.pt"
    ...
    def process_document_batched(self, mentions, features):
        ...
        for i in range(1, len(mentions)):
            ...
            if current_mention.type_ == AnnotationTypes.TOKEN:  # Pronoun logic
        ...
        for i in range(1, len(mentions)):
            ...
            start_idx = max(0, i - self._max_antecedents)
            if mentions[i].type_ == AnnotationTypes.TOKEN:
                start_idx = max(0, i - (self._max_antecedents // 2))
    ...
    def perform_coref(self, document: Document):
        ...
        for token in document.text.tokens:
            if (
                ...
                and (
                    is_person_pronoun(token.content, "en")
                    or token.content.lower() in ("it", "its")
                )
            ):
        ...
        for cluster in resolved_mentions:
            ...
            if (
                ...
                and canonical_mention.type_ == AnnotationTypes.NOUN_CHUNK
            ):
                continue

            if canonical_mention.type_ == AnnotationTypes.TOKEN:
                continue

            if canonical_mention.type_ != AnnotationTypes.ENTITY:
                canonical_mention = document.text.add_annotation(
                    content=canonical_mention.content,
                    start=canonical_mention.start,
                    end=canonical_mention.end,
                    sentence_index=canonical_mention.sentence_index,
                    type_="entity",
            ...
            for mention in cluster["mentions"]:
                ...
                if (
                    mention.type_ == AnnotationTypes.TOKEN
                    or mention.type_ == AnnotationTypes.NOUN_CHUNK
                ):
                    document.text.add_annotation(
                        ...
                        metadata_json={
                            ...
                            Metadata.COREF_TEXT: canonical_mention.normalized,
                        },
                    )
                else:
                    mention.value = canonical_type
                    mention[Metadata.COREF] = canonical_mention.id
                    mention[Metadata.COREF_TEXT] = canonical_mention.normalized
