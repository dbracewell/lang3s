...
from lang3s.data.embedding_db import EmbeddingDB, LabeledItem
...
class DefaultOnlineClusterer[T](OnlineClusterer[T]):
    def __init__(
        ...
        if n_components >= batch_size:
            print(
                "Reduced dimensions exceeds batch size, setting reduced_dimensions"
                "to max(64, batch_size//2)"
            )
