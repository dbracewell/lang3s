...
class ChartingService:
    ...
    def get_chart_data(self, request: ChartDataRequest) -> ChartResult:
        ...
        if request.y is None:
            ...
            inner_join_on = "X.sentence_aid = Y.sentence_aid"
            if (
            ...
            query = f"""
                        ...
                        COUNT(distinct X.sentence_aid) as sentence_count,
                        COUNT() as mention_count
                        FROM X_MAPPED as X 
...
charting_service: ChartingService = None  # type:ignore


def init_charting_service() -> None:
    global charting_service
    charting_service = ChartingService(get_analytics_db())
...
def shutdown_charting_service() -> None:
    global charting_service
    if charting_service is not None:
        charting_service.shutdown()
