...
async def process_batch(batch):
    ...
    for job_id, files in docs_by_id.items():
        ...
        try:
            docs = pipeline(files, write_to_db=True, tasks=tasks, batch_size=len(files))
                ...
                try:
                    ...
                    if job.completed + job.failed >= job.total:
                        ...
                        try:
                            db = Database()
                            db.refresh_views()
                            logger.info("Completed refreshing views")
                        except Exception as e:
                            logger.error(f"Error constructing materialized views: {e}")
