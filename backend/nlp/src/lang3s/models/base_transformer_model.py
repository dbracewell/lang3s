...
class ForkedBaseModel(nn.Module):
    def __init__(self, base_model_name, unfreeze_last_n=2):
        ...
        if os.path.exists(os.path.join(base_model_name, "nli_compressed.pt")):
            self.nli_compression.load_state_dict(
                torch.load(os.path.join(base_model_name, "nli_compressed.pt"))
            )
        else:
            print(
                "Warning could not find compression layer for nli, using random weights",
                file=sys.stderr,
                flush=True,
            )

        if os.path.exists(os.path.join(base_model_name, "search_layers.pt")):
            self.search_layers.load_state_dict(
                torch.load(os.path.join(base_model_name, "search_layers.pt"))
            )
        else:
            print(
                "Warning could not find search layers for search, using nli weights",
                file=sys.stderr,
                flush=True,
            )

        if os.path.exists(os.path.join(base_model_name, "search_compressed.pt")):
            self.search_compression.load_state_dict(
                torch.load(os.path.join(base_model_name, "search_compressed.pt"))
            )
        else:
            print(
                "Warning could not find compression layer for search, using nli weights",
                file=sys.stderr,
                flush=True,
            )
