...
from lang3s.models.layers.adapter import DoRAActivationAdapter, ParallelLoRAAndDoRAActivationAdapterForSequence
from lang3s.models.layers.attention import FastLocalWindowAttention
    ...
    def __init__(self,
        ...
        if self.use_adapter:
            if self.num_attention_heads > 0:
                self.dora_adapter = ParallelLoRAAndDoRAActivationAdapterForSequence(hidden_size=hidden_size,
                                                                                    rank_dora=dora_rank,
                                                                                    rank_lora=lora_rank)
            else:
                self.dora_adapter = DoRAActivationAdapter(hidden_size=hidden_size, rank=dora_rank)
        ...
        if use_focal_loss:
            self.loss_function = FocalLoss(alpha=weights,
                                           loss_type=loss_type,
                                           reduction="mean")
...
class TokenClassificationHead(nn.Module):
    ...
    def __init__(self,
        ...
        if num_attention_heads > 0:
            self.attention_layer = FastLocalWindowAttention(hidden_size=hidden_size,
                                                            window_radius=window_radius,
                                                            num_heads=num_attention_heads,
                                                            dropout=dropout)

        self.classifier = MLPClassificationHead(num_labels=num_labels, hidden=hidden_size)
