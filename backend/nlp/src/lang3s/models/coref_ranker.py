...
def create_coref_mention(annotation: TextAnnotation):
    ...
    if annotation.type == "entity":
        ont_type = ontology.get_ontology_concept_for_mapping(annotation.mapping)
        if not ont_type:
            ont_type = ""
    ...
    for otype, index in NER_MAP.items():
        if is_ontology_type(ont_type, otype):
            mention["ner_idx"] = index
            break

    gender = head.metadata.get(Metadata.GENDER, None)
    number = head.metadata.get(Metadata.NUMBER, None)
