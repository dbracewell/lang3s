import gzip
import json
from pathlib import Path

from lang3s import config
from lang3s.shared_types.document import Document

DOCUMENTS_DIR = "documents"
ANNOTATIONS_FILE_DIR = "annotations"


class FileStore:
    def __init__(self):
        self._base = Path(config.FILESTORE_ROOT)
        self._base.mkdir(parents=True, exist_ok=True)
        (self._base / DOCUMENTS_DIR).mkdir(parents=True, exist_ok=True)
        (self._base / ANNOTATIONS_FILE_DIR).mkdir(parents=True, exist_ok=True)

    def write_document(self, doc: Document):
        documents_dir = self._base / DOCUMENTS_DIR / f"{doc.id}.json.gz"
        with gzip.open(documents_dir, "wt", encoding="utf-8") as gzip_fp:
            json.dump(doc.to_json(), gzip_fp)  # type: ignore
        return doc.id

    def read_document(self, doc_id: str) -> Document:
        documents_dir = self._base / DOCUMENTS_DIR / f"{doc_id}.json.gz"
        with gzip.open(documents_dir, "rt", encoding="utf-8") as gzip_fp:
            return Document.from_json(json.load(gzip_fp))

    def read_annotation_file(self, doc_id: str) -> Document:
        documents_dir = self._base / ANNOTATIONS_FILE_DIR / f"{doc_id}.json.gz"
        with gzip.open(documents_dir, "rt", encoding="utf-8") as gzip_fp:
            return json.load(gzip_fp)

    def get_analytics_db_path(self):
        return self._base / "analytics.duckdb"


FILE_STORE = FileStore()
