...
from lang3s.models.types import EmbeddingResult, TaskType
from lang3s.nlp.event_extraction import extract_events
from lang3s.shared_types import Document
from lang3s.shared_types.core_types import TextAnnotation
...
def heavy_nlp(doc: Document, tasks: Optional[Iterable[str]]):
  embedder = Embedder()
...
def perform_heavy_tagging(
  ...
  for source_name, output in outputs.items():
    if output.task_type == TaskType.SENTENCE:
      for label, sentence in zip(output.labels, doc.text.sentences):
        sentence.metadata[output.annotation_type] = label
        sentence.metadata[f"{output.annotation_type}_source"] = (
          source_name
        )
    else:
  ...
  for event in events:
    ...
    doc.text.add_annotation(
      ...
      metadata={
        ...
        "A0": [a0.id for a0 in event.A0],
        "A0_text": [a0.text for a0 in event.A0],
        "A1": [a1.id for a1 in event.A1],
        "A1_text": [a1.text for a1 in event.A1],
        "TIME": event.TIME.id if event.TIME is not None else None,
        "LOC": event.LOC.id if event.LOC is not None else None,
        "TIME_text": event.TIME.text
        if event.TIME is not None
        else None,
        "LOC_text": event.LOC.text if event.LOC is not None else None,
      },
    )
