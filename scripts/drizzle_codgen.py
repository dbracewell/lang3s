...
PY_MODELS_PATH = (
    REPO_ROOT / "backend" / "nlp" / "src" / "lang3s" / "data" / "db" / "models.py"
)
    ...
    for t in tables_json:
        ...
        for c in t.get("columns", []):
            ...
            col = ColumnSpec(
                ...
                defaultRandom=c.get("defaultRandom", False),
                array=c.get("array", True),
...
def render_sqlalchemy(enums: List[EnumSpec], tables: List[TableSpec]) -> str:
    ...
    lines.append(
        ...
        "Float, Numeric, Text, JSON, Enum, ForeignKey, ARRAY, UUID, text"
        ")"
    )
    ...
    lines.append("from pgvector.sqlalchemy import Vector, HALFVEC")
    lines.append("from lang3s import config")
    lines.append("")
    ...
    for table in tables:
        ...
        for col in table.columns:
            ...
            if col.enum_var and col.enum_var in enum_by_var:
                ...
                if sa_type == "Vector":
                    if col.vector_dims is not None:
                        ...
                        type_expr = "Vector(config.SEMANTIC_EMBEDDING_DIMENSION)"
                elif sa_type == "HALFVEC":
                    if col.vector_dims is not None:
                        type_expr = f"HALFVEC({col.vector_dims})"
                    else:
                        type_expr = "HALFVEC(config.SEMANTIC_EMBEDDING_DIMENSION)"
                else:
                    type_expr = sa_type

            if col.array:
                type_expr = f"ARRAY({type_expr})"
            ...
            if col.defaultRandom and col.drizzle_type == "uuid":
                col_kwargs.append("server_default=text('gen_random_uuid()')")
        ...
        if any(c.name == "id" for c in table.columns):
            ...
            lines.append('        return "<%s>" % self.__class__.__name__')
        lines.append("")
